...
@Log4j2
@RestController
@RequestMapping(value = "sysConfig/scheduler")
public class ScheduleJobController extends BaseController {
     ...
     */
    @Autowired
    private IScheduleJobService serviceObject;
    ...
    public String listData(HttpServletResponse response, HttpServletRequest request) {
        try {
            ...
            List<ScheduleJob> entityObject = serviceObject.listData(pager);
...
//            entityObject.setSort(Integer.parseInt(jsonData.getString("sort")));
    ...
    public ModelAndView gotoConfPage() {
        ...
        try {
            modelAndView.setViewName(UrlPathConst.STR_SYSTEM_CONFIG_JOB_SCHEDULE_JOB_CONF);
        } catch (Exception ex) {
            log.error("XERP Exception:" + ex.toString());
