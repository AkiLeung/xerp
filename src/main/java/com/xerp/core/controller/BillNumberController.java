...
@Log4j2
@RestController
@RequestMapping(value = "sysConfig/billNumber")
public class BillNumberController extends BaseController {

    /**
     * Service操作對象 自動註解
     */
    @Autowired
    private IBillNumberService serviceObject;
    ...
    public String listData(HttpServletResponse response, HttpServletRequest request) {
        try {
            ...
            List<BillNumber> entityObject = serviceObject.listData(pager);
    ...
    public String listByUuid(@RequestParam(value = "uuid") String uuid,
        ...
        try {
            //獲取指定的數據對象到JSON
            List<BillNumber> entityObject = serviceObject.listByUuid(uuid);
    ...
    public String listByCode(@RequestParam(value = "code") String code,
        ...
        try {
            //獲取指定的數據對象到JSON
            JSONObject result = new JSONObject();
            List<BillNumber> entityObject = serviceObject.listByCode(code);
     ...
     * @date 20181108
     */
    @RequestMapping(value = "getModuleName.action")
    @ResponseBody
    public String getModuleName(@RequestParam(value = "code") String code,
                             HttpServletResponse response) {
        try {
            //獲取指定的數據對象到JSON
            List<BillNumber> entityObject = serviceObject.listByCode(code);
    ...
    public String saveData(@RequestBody String strJson,
        ...
        try {
            ...
            //操作對象
            BillNumber entityObject = new BillNumber();
            //uuid
            ...
            entityObject.setModuleCode(jsonData.getString("moduleCode"));
            entityObject.setModuleName(jsonData.getString("moduleName"));
            entityObject.setPreNumWord(jsonData.getString("preNumWord"));
            entityObject.setIncrementRange(jsonData.getString("incrementRange"));
            entityObject.setDefaultLength(Integer.valueOf(jsonData.getString("defaultLength")));
            if (webStatus.equals(ConfigConst.STR_WS_CREATE)) {
    ...
    public ModelAndView gotoConfPage() {
        ...
        try {
            modelAndView.setViewName(UrlPathConst.STR_SYSTEM_CONFIG_BILL_NUMBER_CONF);
        } catch (Exception ex) {
            log.error("XERP Exception:" + ex.toString());
