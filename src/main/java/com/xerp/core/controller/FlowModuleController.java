...
@Log4j2
@RestController
@RequestMapping(value = "sysConfig/flowModule")
public class FlowModuleController extends BaseController {
     ...
     */
    @Autowired
    private IFlowModuleService serviceObject;
    ...
    public String listData(HttpServletResponse response, HttpServletRequest request) {
        try {
            ...
            List<FlowModule> entityObject = serviceObject.listData(pager);
    ...
    public String listByUuid(@RequestParam(value = "uuid") String uuid,
        ...
        try {
            //獲取指定的數據對象到JSON
            List<FlowModule> entityObject = serviceObject.listByUuid(uuid);
    ...
    public String listByCode(@RequestParam(value = "code") String code,
        ...
        try {
            //獲取指定的數據對象到JSON
            JSONObject result = new JSONObject();
            List<FlowModule> entityObject = serviceObject.listByCode(code);
    ...
    public String getZTree(HttpServletResponse response) throws Exception {
        ...
        try {
            nodeList = serviceObject.flowModuleZTree();
        } catch (Exception ex) {
            log.error("XERP Exception:" + ex.toString());
        }

        //設置節點基礎信息
        nodeList = StringUtils.modifyNode(nodeList, "", "", "flowModule", "");
    ...
    public String saveData(@RequestBody String strJson,
        ...
        try {
            ...
            //操作對象
            FlowModule entityObject = new FlowModule();
            //uuid
            ...
            entityObject.setSort(jsonData.getString("sort"));
            if (webStatus.equals(ConfigConst.STR_WS_CREATE)) {
    ...
    public ModelAndView gotoConfPage() {
        ...
        try {
            modelAndView.setViewName(UrlPathConst.STR_SYSTEM_CONFIG_FLOW_MODULE_CONF);
        } catch (Exception ex) {
            log.error("XERP Exception:" + ex.toString());
