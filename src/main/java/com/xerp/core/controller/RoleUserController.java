...
@Log4j2
@RestController
@RequestMapping(value = "sysConfig/roleUser")
public class RoleUserController extends BaseController {
     ...
     */
    @Autowired
    private IRoleUserService serviceObject;
    ...
    public String listData(@RequestParam(value = "roleCode") String roleCode,
        ...
        try {
            ...
            List<RoleUser> dataList = serviceObject.listData(pager);
    ...
    public String saveData(@RequestBody String strJson,
        ...
        try {
            ...
            String[] userCodes = jsonData.getString("userCodes").split(",");
            //循环插入数据
            int check = 0;
            for (int i = 0; i < userCodes.length; i++) {
                check = serviceObject.listByCode(roleCode, userCodes[i]);
                if (check == 0) {
                    //操作對象
                    RoleUser entityObject = new RoleUser();
                    entityObject.setUuid(StringUtils.createUUID());
                    entityObject.setRoleCode(roleCode);
                    entityObject.setUserCode(userCodes[i]);
    ...
    public ModelAndView gotoConfPage() {
        ...
        try {
            modelAndView.setViewName(UrlPathConst.STR_SYSTEM_CONFIG_ROLE_USER_ASSIGN);
        } catch (Exception ex) {
            log.error("XERP Exception:" + ex.toString());
