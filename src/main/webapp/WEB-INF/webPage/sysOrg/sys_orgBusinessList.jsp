...
<div class="easyui-layout" data-options="fit:true">
            ...
            <tr>
                <td bgColor="#F9FBFC"
                    ...
                    &nbsp;<img alt="Config" src="<%=basePath%>static/image/config.gif"
                               style="width: 16px; height: 16px"/>&nbsp;事业部列表
                </td>
            </tr>
            <tr>
    ...
    <div id="businessConfig" class="easyui-dialog" style="width:700px;height:400px"
         data-options="
         ...
         buttons: [{
					...
					iconCls:'icon-save',
					handler:function(){
                        $('#ifrBusinessConfig')[0].contentWindow.saveData();
					}
				},{
					text:'Close',
					iconCls:'icon-close',
					handler:function(){
						$('#businessConfig').dialog('close');
					}
				}],cls:'c8'">
        <iframe id="ifrBusinessConfig" name="ifrBusinessConfig" src="" width="100%" height="100%"
...
<script type="text/javascript">
    ...
    var toolbar = [{
        ...
        iconCls: 'icon-add',
        handler: function () {
            showDialogWin('ifrBusinessConfig','businessConfig',
                '<%=basePath %>sysOrg/business/gotoConfPage.action?ws=<%=ConfigConst.STR_WS_CREATE%>');
        }
    }, '-', {
        text: 'Remove',
        iconCls: 'icon-remove',
        handler: function () {
            ...
            $.messager.confirm("Message", "Are Sure to this option?", function (r) {
                if (r) {
                    $.post("<%=basePath%>sysOrg/business/deleteData.action", {
                        uuids: arrayIds
                    }, function (result) {
                        if (result.success) {
    ...
    $(function () {
        $('#dataList').datagrid({
            url: '<%=basePath%>sysOrg/business/listData.action',
            columns: [[
