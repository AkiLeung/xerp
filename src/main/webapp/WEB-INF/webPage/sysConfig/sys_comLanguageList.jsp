    ...
    var toolbar = [{
        ...
        handler: function () {
            showDialogWin('ifrComLanguageConfig', 'comLanguageConfig',
                '<%=basePath %>sysConfig/comLanguage/gotoConfPage.action?ws=<%=ConfigConst.STR_WS_CREATE%>');
        }
    }, '-', {
        text: 'Remove',
        iconCls: 'icon-remove',
                ...
                {field: 'cb', checkbox: 'true', width: 30},
                {
                    ...
                    formatter: function (value, row, index) {
                        return '<a style="color:blue" href="#" onclick=showDialogWin("ifrComLanguageConfig","comLanguageConfig",' +
                            '"<%=basePath %>sysConfig/comLanguage/gotoConfPage.action?ws=update&uuid=' + row.uuid + '");>'
                            + row.type + '</a>';
                    }
                },
                {
