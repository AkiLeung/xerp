...
<form id="form1" action="" method="post">
    ...
    <div data-options="region:'center'">
        <table id="dataList" class="easyui-datagrid" title="用户清单" style="width:885px;height:623px"
               data-options="
               rownumbers:true,
               idField:'uuid',
               pagination:true,
               ...
               pageList:[20,30,50],
               collapsible:true,
               url:'<%=basePath%>sysConfig/roleUser/listData.action?roleCode=<%=request.getParameter("roleCode")%>',
               method:'get',
               toolbar:toolbar">
            <thead>
            <tr>
                ...
                <th data-options="field:'userCode',width:150">用户编码</th>
                <th data-options="field:'userName',width:140">用户名称</th>
    ...
    <div id="assignUser" class="easyui-dialog" style="width:90%;height:650px"
         data-options="
         ...
         buttons: [{
					...
					iconCls:'icon-save',
					handler:function(){
                        $('#ifrAssignUser')[0].contentWindow.saveData();
					}
				},{
					text:'Close',
					iconCls:'icon-close',
        ...
        <iframe id="ifrAssignUser" name="ifrAssignUser" src="" width="100%" height="100%"
...
<script>
    ...
    var toolbar = [{
        ...
        iconCls: 'icon-add',
        handler: function () {
            showDialogWin('ifrAssignUser', 'assignUser',
                '<%=basePath %>sysConfig/roleUser/gotoAssignPage.action?roleCode=<%=request.getParameter("roleCode")%>');
        }
    }, '-', {
        text: '删除',
        iconCls: 'icon-remove',
        handler: function () {
            ...
            $.messager.confirm("Message", "Are Sure to this option?", function (r) {
                if (r) {
                    $.post("<%=basePath%>sysConfig/roleUser/deleteData.action", {
                        uuids: uuids
                    }, function (result) {
                        if (result.success) {
