...
<script type="text/javascript">
    ...
    $(function () {
        $('#dataList').datagrid({
            url: '<%=basePath%>sysConfig/user/listData.action?search='+$("#search").val(),
            columns: [[
                {field: 'cb', checkbox: 'true', width: 30},
                {field: 'uuid', title: 'uuid', width: 100, hidden: true},
                {field: 'userCode', title: '权限编码', width: 150},
                {field: 'userName', title: '权限名稱', width: 150}
            ]],
    ...
    function saveData() {
        ...
        for (var i = 0; i < selectedRows.length; i++) {
            arrayIds.push(selectedRows[i].userCode);
        }
        var userCodes = arrayIds.join(",");
        //執行保存
        var objData = {
        ...
        $.ajax({
            type: "POST",
            url: "<%=basePath %>sysConfig/roleUser/saveData.action",
            dataType: "json",
            contentType: 'application/json;charset=UTF-8',
            async: false,
            data: jsonData,
            success: function (data) {
                parent.$('#assignUser').dialog('close');
    ...
    //查询
    function doSearch(){
        var url='<%=basePath%>sysConfig/user/listData.action?search='+$("#search").val();
