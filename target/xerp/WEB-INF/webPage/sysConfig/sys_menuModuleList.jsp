...
<div class="easyui-layout" data-options="fit:true">
            ...
            <tr>
                <td bgColor="#F9FBFC"
                    ...
                    &nbsp;<img alt="Config" src="<%=basePath%>static/image/config.gif"
                               style="width: 16px; height: 16px"/>&nbsp;菜單模塊
                </td>
            </tr>
            <tr>
    ...
    <div id="menuModuleConfig" class="easyui-dialog" style="width:900px;height:350px"
         data-options="
         ...
         buttons: [{
					...
					iconCls:'icon-save',
					handler:function(){
                        $('#ifrMenuModuleConfig')[0].contentWindow.saveData();
                        $('#menuModuleConfig').dialog('close');
        ...
        <iframe id="ifrMenuModuleConfig" name="ifrMenuModuleConfig" src="" width="100%" height="100%"
...
<script type="text/javascript">
    ...
    var toolbar = [{
        ...
        iconCls: 'icon-add',
        handler: function () {
            showDialogWin('ifrMenuModuleConfig', 'menuModuleConfig',
                '<%=basePath %>sysConfig/menuModule/gotoConfPage.action?ws=<%=ConfigConst.STR_WS_CREATE%>');
        }
    }, '-', {
        text: '删除',
        iconCls: 'icon-remove',
        handler: function () {
            ...
            $.messager.confirm("Message", "Are Sure to this option?", function (r) {
                if (r) {
                    $.post("<%=basePath%>sysConfig/menuModule/deleteData.action", {
                        uuids: uuids
                    }, function (result) {
                        if (result.success) {
                            $("#dataList").datagrid("reload");
    ...
    $(function () {
        $('#dataList').datagrid({
            url: '<%=basePath%>sysConfig/menuModule/listData.action',
            columns: [[
                ...
                {field: 'statusTxt', title: '狀態', width: 100},
                {
                    ...
                    formatter: function (value, row, index) {
                        return '<a style="color:blue" href="#" onclick=showDialogWin("ifrMenuModuleConfig","menuModuleConfig",' +
                            '"<%=basePath %>sysConfig/menuModule/gotoConfPage.action?ws=update&uuid=' + row.uuid + '");>'
                            + row.moduleCode + '</a>';
                    }
                },
                {
